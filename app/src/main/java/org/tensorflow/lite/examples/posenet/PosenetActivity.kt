...
class PosenetActivity :
  ...
  private val bodyJointA = listOf(
    ...
    Triple(BodyPart.RIGHT_KNEE, BodyPart.RIGHT_HIP,BodyPart.LEFT_HIP),
    Triple(BodyPart.LEFT_KNEE, BodyPart.LEFT_HIP,BodyPart.RIGHT_HIP)
  )
  ...
  private fun draw(canvas: Canvas, person: Person, bitmap: Bitmap) {
    ...
    for (line in bodyJointA) {
      if (
        ...
        var angle: Float = atan((position1.y.toFloat()-position2.y.toFloat())/(position1.x.toFloat()-position2.x.toFloat()))-atan((position3.y.toFloat()-position2.y.toFloat())/(position3.x.toFloat()-position2.x.toFloat()))
        Angle.add(angle)
      }
    }
    ...
    if(Angle.size>=8) {
      canvas.drawText(
              "왼팔: %.2f".format(Angle[0]*57),
              (15.0f * widthRatio),
              (10.0f * heightRatio + bottom),
              paint
      )
      canvas.drawText(
              "왼쪽어깨:%.2f".format(Angle[1]*57),
              (15.0f * widthRatio),
              (20.0f * heightRatio + bottom),
              paint
      )
      canvas.drawText(
        "오른팔:%.2f".format(Angle[2]*57),
        (15.0f * widthRatio),
        (30.0f * heightRatio + bottom),
        paint
      )
      canvas.drawText(
        "오른쪽어깨:%.2f".format(Angle[3]*57),
        (15.0f * widthRatio),
        (40.0f * heightRatio + bottom),
        paint
      )
      canvas.drawText(
        "왼쪽무릎:%.2f".format(Angle[4]*57),
        (15.0f * widthRatio),
        (50.0f * heightRatio + bottom),
        paint
      )
      canvas.drawText(
        "오른쪽무릎:%.2f".format(Angle[5]*57),
        (15.0f * widthRatio),
        (60.0f * heightRatio + bottom),
        paint
      )
      canvas.drawText(
        "오른쪽골반:%.2f".format(Angle[6]*57),
        (15.0f * widthRatio),
        (70.0f * heightRatio + bottom),
        paint
      )
      canvas.drawText(
        "왼쪽골반:%.2f".format(Angle[7]*57),
        (15.0f * widthRatio),
